 ...
 */
public class Report1Task implements Runnable {
    ...
    private static final Logger logger = LogManager.getLogger(Report1Task.class);
    ...
    public Report1Task(File file, String bbname, String bbtype, JdbcTemplate jdbcTemplate, LocalDate rightnow) throws IOException {
        this.bbname = bbname;
        this.bbtype = bbtype;
     ...
     * @param saveno
     * @param zero
     * @param type    预留：0：时平均值，1，时最大值，2：时最小值，3：整点取值
     * @param rightnow
     * @return
     */
    public JSONObject getYcbbData(JdbcTemplate jdbcTemplate,String saveno,String zero,int type, LocalDate rightnow){
        ...
        float avg=0.0f;
        String sql="select (floor(savetime/100) mod 100) dd,ROUND(avg(val"+vno+"),2) vv from hyc"+(rightnow.getYear()%10)+" where ABS(val"+vno+")"+zero_condition+zero+" and  groupno="+gno+" and savetime between "+df.format(rightnow)+"0000 and "+df.format(rightnow)+"2400 group by (floor(savetime/100) mod 100) ";
//        logger.warn(sql);
        ...
        sql="select maxv,maxt,minv,mint from everyday  where saveno="+saveno+" and DATE_FORMAT(tdate,'%Y%m%d')='"+df2.format(rightnow)+"'";
//        logger.warn(sql);
        //if(rand.equalsIgnoreCase(imagerand)){
        try{
            List<Map<String, Object>> userData = jdbcTemplate.queryForList(sql);
    ...
    public void run()  {
        ...
        String exportFilePath = reportPath+"/"+rightnow.getYear()+"/"+rightnow.getMonthValue()+"/"+rightnow.getDayOfMonth()+"/"+bbname;
